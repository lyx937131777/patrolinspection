...
public class PsamL2HUtil
{
    private static final String TAG = "PsamL2HUtil";
    private boolean DEBUG = true;

    private PSAMReaderManager prm;
    private byte[] returnValue;
    private int returnValueLength;
    ...
    public PsamL2HUtil(Context c){
        //mMiscDevice = new MiscDevice("/dev/psam",'M');
        prm = PSAMReaderManager.getInstance(c);
        prm.setPSAMReaderListener(mListener);
        context = c;
    }
...
//        Tools.system("echo 0 > /sys/class/misc/psam/power_en");
    ...
    public boolean resetPsam(){
        ...
        try{
            ...
            LogUtil.e(TAG,"reset cmd byte: " + cmd);
            if(DEBUG) Log.e(TAG, "cmd =" + CommonUtil.bytesToHexString(cmd));
//            mTtyDevice.ttyWrite(cmd);
            prm.sendApduCmd(cmd);
            CommonUtil.delayms(500);
            byte[] temp = returnValue;
            int ret = returnValueLength;
    ...
    public byte[] sendDataToPsam(String data){
        try {
            if(DEBUG) Log.e(TAG, "send data =" + data);
//            byte[] cmd = getApduCmdPacket(CommonUtil.hexStringToBytes(data));
            byte[] cmd = CommonUtil.hexStringToBytes(data);
            LogUtil.e(TAG,"cmd byte: " + cmd);
            if(DEBUG) Log.e(TAG, "cmd =" + CommonUtil.bytesToHexString(cmd));
//            mTtyDevice.ttyWrite(cmd);
            prm.sendApduCmd(cmd);

            LogUtil.e(TAG,"send CMD !!");
            CommonUtil.delayms(500);
            byte[] temp = returnValue;
            LogUtil.e(TAG,"函数内： 返回值为： " + temp);
            int ret = returnValueLength;
...
//      String hjxz = new String(basicInfo, 109, 70);
    ...
    private PSAMReaderManager.PSAMReaderListener mListener = new PSAMReaderManager.PSAMReaderListener(){

        @Override
        public void onApduCmdReturn(int state,byte[] value,String errorMsg){
            String str;
            if(state != PSAMReaderManager.READER_STATE_OK){
                str = errorMsg;
                LogUtil.e(TAG,"返回错误！！   "  + str);
            }else{
                returnValueLength = value.length;
                returnValue = value;
                LogUtil.e(TAG,"返回值" + value);
            }
        }
